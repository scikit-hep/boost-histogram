...
//         {static_cast<std::size_t>(ax.size() + underflow + overflow),
//         std::size_t(2)});
//
//     for(auto i = -underflow; i < ax.size() + overflow; ++i) {
//         result.mutable_at(static_cast<std::size_t>(i + underflow), 0) = ax.value(i);
//         result.mutable_at(static_cast<std::size_t>(i + underflow), 1) = ax.value(i +
//         1);
//     }
//
...
//         = flow && (bh::axis::traits::options(ax) & option::overflow);
//
//     py::array_t<int> result(static_cast<std::size_t>(ax.size() + underflow +
//     overflow));
//
//     for(auto i = -underflow; i < ax.size() + overflow; ++i)
...
// py::array bins_impl(const bh::axis::category<U, Ts...> &ax, bool flow) {
//     static_assert(!(std::decay_t<decltype(ax)>::options() & option::underflow),
//                   "discrete axis never has underflow");
//
//     std::vector<py::object> v;
//     const auto n = ax.size() + flow;
//     v.reserve(static_cast<unsigned>(n));
//     for(int i = 0; i < n; ++i)
//         v.emplace_back(unchecked_bin(ax, i));
//
//     return py::cast(v);
// }
//
// template <class Options>
// py::array bins_impl(const category_str_t<Options> &ax, bool flow) {
//     return bins_impl(static_cast<const typename category_str_t<Options>::base_t
//     &>(ax),
//                      flow);
// }
//
